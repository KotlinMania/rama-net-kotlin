# Claude Code Project Instructions — rama-net-kotlin

## Project Overview

This is **rama-net-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`rama-net`](https://crates.io/crates/rama-net).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/rama-net/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/rama-net>
